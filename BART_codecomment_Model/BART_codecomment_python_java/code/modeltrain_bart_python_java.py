# File: code/modeltrain_bart_python_java.py 
...
def train_bart_model(train_dataset_path, valid_dataset_path, output_base_dir, model_checkpoint='facebook/bart-base', seed=42):
    ...
    model_output_dir = os.path.join(output_base_dir, 'models', 'BART-finetuned_model')
    checkpoint_dir = os.path.join(output_base_dir, 'models', 'BART_training_checkpoints')
...
# === [3] Argument Parser ===
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune BART model for code-comment generation.")
    ...
    parser.add_argument('--output_base_dir', type=str, required=True, help='Directory to save model and logs.')
