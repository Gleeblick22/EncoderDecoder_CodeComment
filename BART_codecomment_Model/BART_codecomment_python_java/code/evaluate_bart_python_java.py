# File: code/evaluate_bart_python_java.py
...
def evaluate_model(predictions_path, test_path, output_dir):
    ...
    # Save scores
    scores_path = os.path.join(output_dir, "evaluation_bart_python_java_scores.json")
    ...
    qual_path = os.path.join(output_dir, "bart_python_java_qualitative_examples.txt")
    with open(qual_path, 'w', encoding='utf-8') as f:
        f.write("Model Evaluated: BART-base Python + Java\n")
        f.write("=" * 70 + "\n\n")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate BART model-generated comments")
