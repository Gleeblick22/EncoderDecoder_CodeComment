...
#File: code/inference_bart_python.py
...
def save_metadata(output_dir, model_dir, test_file):
    metadata = {
        "Model Evaluated": "BART-base Python-only",
        "Model Path": model_dir,
        "Test File": test_file
    }
    metadata_path = os.path.join(output_dir, "bart_python_metadata.json")
    with open(metadata_path, "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=4)
    print(f"Metadata saved to: {metadata_path}")

def parse_args():
    parser = argparse.ArgumentParser(description="Generate comments using Python-only fine-tuned BART model")
    parser.add_argument("--model_dir", type=str, required=True, help="Path to fine-tuned BART model directory")
    parser.add_argument("--test_dataset_path", type=str, required=True, help="Path to Python test JSON file")
    parser.add_argument("--output_base_dir", type=str, required=True, help="Directory to save generated results")
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(args.output_base_dir, "bart_python_generated_comments.json")
