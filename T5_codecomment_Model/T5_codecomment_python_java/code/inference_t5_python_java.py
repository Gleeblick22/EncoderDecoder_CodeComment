# File: inference_t5_python_java.py
...
def save_metadata(output_dir, model_dir, test_file):
    metadata = {
        "Model Evaluated": "T5 Python + Java",
        "Model Path": model_dir,
        "Test File": test_file
    }
    metadata_path = os.path.join(output_dir, "t5_python_java_metadata.json")
    with open(metadata_path, "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=4)
    print(f" Metadata saved to: {metadata_path}")

def parse_args():
    parser = argparse.ArgumentParser(description="Generate comments using fine-tuned T5 model")
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(args.output_base_dir, "t5_python_java_generated_comments.json")
