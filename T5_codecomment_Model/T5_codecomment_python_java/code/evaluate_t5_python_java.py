# File: code/evaluate_t5_python_java.py
...
def main(predictions_path, test_path, output_dir):
    ...
    with open(os.path.join(output_dir, "evaluation_t5_python_java_scores.json"), "w", encoding="utf-8") as f:
    ...
    qual_path = os.path.join(output_dir, "t5_python_java_qualitative_examples.txt")
    with open(qual_path, 'w', encoding='utf-8') as f:
        f.write("Model Evaluated: T5 Python + Java\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate T5 model on Python+Java dataset")
    parser.add_argument('--predictions_path', type=str, required=True, help="Path to generated comments JSON file")
    parser.add_argument('--test_path', type=str, required=True, help="Path to test JSON with code and comments")
