# File: code/inference_t5_java.py

# File: inference_t5_java.py
...
def save_metadata(output_dir, model_dir, test_file):
    metadata = {
        "Model Evaluated": "T5 Java Only",
        "Model Path": model_dir,
        "Test File": test_file
    }
    metadata_path = os.path.join(output_dir, "t5_java_metadata.json")
    with open(metadata_path, "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=4)
    print(f" Metadata saved to: {metadata_path}")

def parse_args():
    parser = argparse.ArgumentParser(description="Generate comments using fine-tuned T5 model for Java")
    parser.add_argument("--model_dir", type=str, required=True, help="Path to T5 model directory")
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(args.output_base_dir, "t5_java_generated_comments.json")
